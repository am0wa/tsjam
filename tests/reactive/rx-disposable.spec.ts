/* eslint-disable @typescript-eslint/explicit-function-return-type,functional/prefer-readonly-type,@typescript-eslint/no-unsafe-assignment */
...
describe('RxDisposable',() => {
  ...
  it('dispose - to avoid extra invocations when disposed', () => {
    ...
    let invocations = 0;
    const cbD = { unsubscribe: () => { invocations++ } };
  ...
  it('autoDispose to support rx and functional teardown', () => {
    ...
    class RxDisposableTest extends RxDisposable {
      ...
      constructor() {
        ...
        // add streams
        this.subA = this.autoDispose(
          streamA$.subscribe(v => valueA = v)
        )
        // add disposables - and assign instance in one line
        this.child = this.autoDispose(
          new TestDisposable()
        );
